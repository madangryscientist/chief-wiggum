...
import { $ } from "bun";
import { existsSync, readFileSync, writeFileSync, mkdirSync, statSync, unlinkSync, copyFileSync, appendFileSync } from "fs";
...
function parseStructuredTasks(content: string): ParsedTasksFile {
  ...
  for (const line of lines) {
    ...
    if (milestoneMatch) {
      ...
      if (!milestones.has(currentMilestone)) milestones.set(currentMilestone, []);
      continue;
    }
    ...
    if (taskMatch) {
      ...
      currentTask = {
        id, title, milestone: currentMilestone, status,
        depends: [], verify: null, started: null, completed: null, originalLines: [],
      };
      taskLines = [line];
      continue;
    }
...
function ensureRalphConfig(options: { filterPlugins?: boolean; allowAllPermissions?: boolean }): string {
  ...
  if (options.allowAllPermissions) {
    config.permission = {
      ...
      codesearch: "allow", todowrite: "allow", todoread: "allow", question: "allow",
      lsp: "allow", external_directory: "allow",
    };
  }
...
async function setupWorktree(
  ...
  if (tasksFilePath && existsSync(tasksFilePath)) {
    ...
    if (targetTasksDir && !existsSync(targetTasksDir)) mkdirSync(targetTasksDir, { recursive: true });
...
async function streamProcessOutput(
...
): Promise<{ stdoutText: string; stderrText: string; toolCounts: Map<string, number>; timedOut: boolean }> {
  ...
  const maybePrintToolSummary = (force = false) => {
    ...
    const now = Date.now();
    if (!force && now - lastToolSummaryAt < options.toolSummaryIntervalMs) return;
  ...
  const heartbeatTimer = setInterval(async () => {
    ...
    if (now - lastPrintedAt >= options.heartbeatIntervalMs) {
      ...
      const timeoutIn = options.inactivityTimeoutMs > 0 ? ` · timeout in ${formatDuration(options.inactivityTimeoutMs - inactivityDuration)}` : "";
      console.log(`⏳ working... elapsed ${elapsed} · last activity ${sinceActivity} ago${timeoutIn}`);
      lastPrintedAt = now;
    }
...
function extractErrors(output: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      lower.includes("typeerror") || lower.includes("syntaxerror") || lower.includes("referenceerror") ||
...
`);
  ...
  if (state?.active) {
    ...
    if (state.structuredTasksFile) {
      ...
      if (state.milestoneFilter) console.log(`   Milestone:    ${state.milestoneFilter}`);
    }
  } else {
...
function cmdContext(args: string[], flags: Record<string, string | boolean>): void {
  if (flags.workspace) workspaceRoot = flags.workspace as string;
...
function cmdTasks(args: string[], flags: Record<string, string | boolean>): void {
  if (flags.workspace) workspaceRoot = flags.workspace as string;
  ...
  if (subcommand === "rm") {
    ...
    if (taskIndex < 1 || taskIndex > tasks.length) {
      console.error(`Error: Index ${taskIndex} out of range (1-${tasks.length})`);
      process.exit(1);
    }
...
async function cmdRun(args: string[], flags: Record<string, string | boolean>): Promise<void> {
  ...
  const opts: RunOptions = {
    ...
    iterations: parseInt((flags.iterations as string) || "0") || 0,
    ...
    timeout: parseInt((flags.timeout as string) || "30") || 30,
    force: !!flags.force,
...
`);
  ...
  while (true) {
    ...
    try {
      ...
      if (suggestedTasks.length > 0) appendSuggestedTasks(suggestedTasks, state.iteration);
      ...
      if (iterationDuration < 30000) history.struggleIndicators.shortIterations++;
